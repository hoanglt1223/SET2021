    ...
    return fileSystemDataSource
      ...
      .then((data) => {
        if (where && Array.isArray(data) && data.length > 0) {
          const filteredData = data.filter((item) => Object.keys(where).every((field) => where[field] === item[field]))
          return filteredData
        }
        return data
      })
      ...
      if (validationError) {
        ...
        resolve(
          this.find().then((existingItems) => {
            ...
            return fileSystemDataSource.updateCollection(DBCollections[name], existingItems).catch((err) => {
        ...
        resolve(
          this.find().then((existingItems) => {
            ...
            existingItems.forEach((element) => {
              if (element.id == newItem.id) element.taskName = newItem.taskName
            })
            return fileSystemDataSource.updateCollection(DBCollections[name], existingItems).catch((err) => {
